...
precompiled_installer_download_url() {
  ...
  local -r milestone="$2"
  # 11895.86.0 -> 11895-86-0
  local -r build_id="${3//\./-}"
  local -r download_location="$(get_download_location)"

  echo "https://storage.googleapis.com/nvidia-drivers-${download_location}-public/nvidia-cos-project/tesla/${milestone}/${major_version}_00/${driver_version}/NVIDIA-Linux-x86_64-${driver_version}_${milestone}-${build_id}.cos"
}
...
get_gpu_installer_url() {
  if [ ! -v GPU_INSTALLER_DOWNLOAD_URL ]; then
    # First try to find the precompiled gpu installer.
    url="$(precompiled_installer_download_url $@)"
