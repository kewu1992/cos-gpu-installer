...
installer_default_download_url() {
  ...
  echo  "https://storage.googleapis.com/kewu_develop_bucket/NVIDIA-Linux-x86_64-410.79-4.14.74.cos"
  return
  # END OF TESTING
...
main() {
  ...
  if check_cached_version; then
    ...
    #download_kernel_src
    time install_cross_toolchain_pkg
    time configure_nvidia_installation_dirs
    time download_nvidia_installer
    #configure_kernel_src
    time run_nvidia_installer
    time update_cached_version
    time verify_nvidia_installation
