...
sign_gpu_driver() {
  ...
  local -r priv_key="${key_dir}/priv.key"
  local -r publ_key="${key_dir}/publ_key.pem"
  export TPM_INTERFACE_TYPE=dev
  export OPENSSL_CONF=/openssl.cnf
  create_tpm2_key --key-size 2048 --rsa -n sha256 "${priv_key}"
  openssl req -new -x509 -batch \
    -days 36500 -sha256 -config "/x509.genkey" \
    -keyform engine -engine tpm2 -key "${priv_key}" \
    -outform pem -out "${publ_key}"

  # Sign kernel modules.
  driver_path="${NVIDIA_INSTALL_DIR_CONTAINER}/drivers"
  for module in "${driver_path}"/*.ko; do
    # Generate raw detached PKCS#7 signature.
    openssl smime -sign -nocerts -noattr \
      -binary -in "${module}" \
      -keyform engine -engine tpm2  \
      -inkey "${priv_key}" -signer "${publ_key}" \
      -outform der -out "${module}.raw_sig"

    # Append signature to the end of the module.
    "${KERNEL_SRC_DIR}"/scripts/sign-file -s "${module}.raw_sig" \
      sha256 "${publ_key}" "${module}"
  ...
  mount "${esp_partition}" "${mount_path}"
  openssl x509 -inform pem -in ${publ_key} \
    -outform der -out "${mount_path}/${uefi_db_path}/gpu_signing_key.der"
  umount "${mount_path}"
}
